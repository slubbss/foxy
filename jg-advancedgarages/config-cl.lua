...
-- Blips
--
local jobGarageBlips, gangGarageBlips = {}, {}
...
function createPublicPrivateGarageBlips()
  CreateThread(function()
    ...
    Framework.Client.TriggerCallback("jg-advancedgarages:server:GetPrivateGarages", function(privateGarages)
      ...
      if Config.GarageShowBlips then
        for id, garage in pairs(Globals.Garages) do
          if not garage.hideBlip then
            ...
            createBlip(blipName, garage.coords, Config.GarageBlipId, Config.GarageBlipColour, Config.GarageBlipScale)
          end
        end
      end
    end, identifier)
      ...
      for id, garage in pairs(Config.JobGarageLocations) do
        if not garage.hideBlip then
          if garage.job == Framework.Client.GetPlayerJob().name then
            ...
            local blip = createBlip(blipName, garage.coords, Config.JobGarageBlipId, Config.JobGarageBlipColour, Config.JobGarageBlipScale)
      ...
      for id, garage in pairs(Config.GangGarageLocations) do
        if not garage.hideBlip then
          if garage.gang == Framework.Client.GetPlayerGang().name then
            ...
            local blip = createBlip(blipName, garage.coords, Config.GangGarageBlipId, Config.GangGarageBlipColour, Config.GangGarageBlipScale)
    ...
    if Config.ImpoundShowBlips then
      for id, impound in pairs(Config.ImpoundLocations) do
        if not impound.hideBlip then
          ...
          createBlip(blipName, impound.coords, Config.ImpoundBlipId, Config.ImpoundBlipColour, Config.ImpoundBlipScale)
        end
      end
    end
  end)
...
local function showDrawText(id, text)
...
local function hideDrawText(id)
